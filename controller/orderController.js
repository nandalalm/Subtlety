...
async function checkoutPage(req, res, next) {
  try {
    ...
    if (error.statusCode === 400 && error.redirect) {
        if (req.headers.accept && req.headers.accept.includes("application/json")) {
...
async function applyCoupon(req, res, next) {
  try {
    ...
    if (error.statusCode === 400) return res.status(error.statusCode).json({ message: error.message });
    next(error);
  }
}
...
async function createRazorpayOrder(req, res, next) {
  try {
    ...
    if (error.statusCode === 400) return res.status(error.statusCode).json({ success: false, message: error.message });
    next(error);
  }
}
...
async function cancelProduct(req, res, next) {
  try {
    ...
    res.json({ success: true, message: MESSAGES.ORDER.CANCELLED });
  } catch (error) {
...
async function returnProduct(req, res, next) {
  try {
    ...
    res.json({ success: true, message: MESSAGES.ORDER.RETURN_SUBMITTED });
  } catch (error) {
    if (error.statusCode === 400 || error.statusCode === 404) {
        return res.status(error.statusCode).json({ success: false, message: error.message });
    }
...
async function getAdminOrderDetails(req, res, next) {
  try {
    ...
    if (error.statusCode === 404) return res.status(error.statusCode).json({ message: error.message });
    next(error);
  }
}
...
async function approveReturn(req, res, next) {
  try {
    ...
    await orderService.handleReturn(orderId, productId, action);
    res.json({
      success: true,
